...
Estos datos, de **CVEs** aplicados a equipos, se almacenan en **OpenSearch/ElasticSearch** por lo que, desde cualquier programa podemos realizar consultas a la **API** y hacer uso de esos datos. 
...
El despliegue se realiza con **Docker**, para desplegarlo simplemente es necesario ejecutar el siguiente comando: 
...
**No es recomendable** agregar un **alertado automático**, ya que es posible que se produzcan **demasiados falsos positivos**. Para que este nivel de **FPs** sea menor, se recomienda seguir entrenando el **modelo** de aprendizaje. 
